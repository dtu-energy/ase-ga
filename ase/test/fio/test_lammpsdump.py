...
def lammpsdump():
    def factory(bounds="pp pp pp",
        ...
        0.0e+00 5.0e+00
        0.0e+00 2.0e+01
        ITEM: ATOMS {_element} {_id} {_type} {position_cols} {_i_property} {_i2_property} {_d_property} {_d2_property}
...
def test_lammpsdump_custom_property(fmt, lammpsdump):
    ...
    assert np.all(atoms.arrays['i2_property[1]'].flatten() == np.array([2, 4, 6]))
    assert np.all(atoms.arrays['d_property'].flatten() == np.array([1., 3., 5.]))
    assert np.all(atoms.arrays['d2_property[1]'].flatten() == np.array([2., 4., 6.]))
